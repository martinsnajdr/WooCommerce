(function ($) {
	...
	$(function () {
		/*
		let $shippingSettingsLink = $('a.wc-shipping-zone-method-settings');
		if ($shippingSettingsLink.length === 0) {
			return;
		}

		$shippingSettingsLink.click(function () {
		 */

		// are we on the right page?
		if ($('.packetery-rule').length === 0) {
			return;
		}

		var Multiplier = function (wrapperSelector) {
			var $wrapper = $(wrapperSelector), multiplier = this;
			...
			function findMaxNewId() {
				var $newInputs = $(wrapperSelector + ' [name*=' + prefix + ']'), maxNewId = 1;
			...
			var prefix = 'new_', newId = findMaxNewId();
			...
			function getTemplateClone(container) {
				...
				*/
				...
				var $template = $('.packetery-hidden table');
				return $template.find(wrapperSelector).find('[data-replication-item]').first().clone(); // table TR is currently the replication item
			}
